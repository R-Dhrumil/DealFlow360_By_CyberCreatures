...
async function getDashboardMetrics(companyId) {
  // Aggregate metrics: total quotes, approved quotes (win rate proxy), pending, and average risk score
  ...
  const total = parseInt(row.total_quotes) || 0;
  const won = parseInt(row.won_quotes) || 0;
  
  return {
    totalQuotes: total,
    winRate: total > 0 ? ((won / total) * 100).toFixed(1) : 0,
    pendingApprovals: parseInt(row.pending_quotes) || 0,
    avgRiskScore: parseFloat(row.avg_risk_score || 0).toFixed(2)
  };
}

async function getHighRiskDeals(companyId, limit = 5) {
  const result = await pool.query(`
