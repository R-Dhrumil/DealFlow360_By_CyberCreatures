    ...
    <div className="min-h-screen bg-slate-50 p-6 md:p-8 space-y-6">
      <header className="flex flex-wrap justify-between items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Live order cart, upsell margin recommendations & governance check</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            {lines.length === 0 ? (
              <div className="text-center py-12 bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl">
                <i className="fa-solid fa-cart-arrow-down text-slate-400 text-4xl mb-3"></i>
                <p className="text-slate-600 font-semibold text-sm">No items in quotation cart.</p>
                <p className="text-xs text-slate-400 mt-1">Select products from the catalog or click quick add below.</p>
              </div>
            ) : (
                    ...
                    {lines.map((line, idx) => {
                      ...
                      return (
                        <tr key={idx} className="hover:bg-slate-50/80">
                          <td className="py-3 text-slate-900 font-semibold text-sm">
                            {line.productName}
                          ...
                          <td className="py-3 text-slate-900 font-bold text-sm">${(netPrice * line.quantity).toFixed(2)}</td>
                      ...
                      <div key={product.id} className="bg-white border border-indigo-100 rounded-xl p-4 flex justify-between items-center shadow-sm">
                        ...
                        <div className="flex flex-col space-y-1 text-right">
                          <button 
                            ...
                            className="text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 px-3 py-1.5 rounded-lg shadow-sm transition-colors flex items-center justify-center"
                          >
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <h2 className="text-base font-bold text-slate-900 mb-4">Product Catalog</h2>
            <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1">
              {products.map(product => (
                <div 
                  key={product.id} 
                  className="border border-slate-200 rounded-lg p-3 hover:border-primary-400 hover:bg-slate-50/60 transition-all cursor-pointer group flex justify-between items-center" 
                  onClick={() => addLine(product)}
                >
                  <div>
                    <h4 className="font-bold text-slate-800 text-xs group-hover:text-primary-600">{product.name}</h4>
                    <span className="text-[11px] text-slate-500">{product.category} &bull; ${parseFloat(product.base_price).toFixed(2)}</span>
                  </div>
                  <button className="w-7 h-7 rounded-lg bg-slate-100 group-hover:bg-primary-600 group-hover:text-white text-slate-500 flex items-center justify-center transition-colors">
